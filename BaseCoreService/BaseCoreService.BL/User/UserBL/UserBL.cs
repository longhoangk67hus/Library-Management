...
namespace BaseCoreService.BL
{
    public class UserBL : BaseBL, IUserBL
    {
        ...
        public override void AfterSave(BaseEntity entity, IDbTransaction transaction)
        {
            base.AfterSave(entity, transaction);
        }
        ...
        public async Task<ServiceResponse> Register(RegisterInfo registerInfo)
        {
            var serviceResponse = new ServiceResponse();
            var role = await _roleBL.GetRoleByCode("USER");
            if (role == null)
            {
                return serviceResponse.OnError(new ErrorResponse()
                {
                    ErrorMessage = "Không tồn tại user role"
                });
            }
            var user = new User()
            {
                ...
                Password = SecurityUtils.HashPassword(registerInfo.Password),
                PhoneNumber = registerInfo.PhoneNumber,
                UserCode = this.GenerateUserCode(),
                UserRoles = new List<UserRole>() { new UserRole() { RoleID = role.RoleID, State = ModelState.Insert } },
                Status = (int)UserStatus.Active,
                State = ModelState.Insert
            };
            var res = await SaveAsync(user);
            return res;

        }
        private string GenerateUserCode()
        {
            var userCode = Utils.GenerateSearchParam();
            return userCode;
        }
    }
}
