...
namespace BaseCoreService.BL
{
    public interface IBaseBL
    {
        ...
        Task<IEnumerable<T>> GetAllAsync<T>(Type type, PagingRequest request);

        Task<List<T>> GetAllObjectAsync<T>(Type type, PagingRequest request);

        //Task<List<object>> GetAllObjectAsync(Type type, PagingRequest request);



        Task<PagingResponse> GetPagingAsync(Type type, PagingRequest pagingRequest);

        Task<int> GetTotalCount(Type type);
        ...
        Task<bool> DoSaveChangesAsync(BaseEntity entity, List<EntityFieldUpdate> fieldUpdates, IDbTransaction transaction);

        ServiceResponse SaveList(IEnumerable<BaseEntity> entities);

        Task<ServiceResponse> SaveAsync(BaseEntity entity);

        Task<bool> DoSaveAsync(BaseEntity entity, IDbTransaction transaction);
