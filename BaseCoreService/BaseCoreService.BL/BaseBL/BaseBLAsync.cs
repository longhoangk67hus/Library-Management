...
namespace BaseCoreService.BL
{
    public partial class BaseBL : IBaseBL
    {
        ...
        public virtual async Task<ServiceResponse> SaveAsync(BaseEntity entity)
        {
            ...
            try
            {
                ...
                if (result)
                {
                    ...
                    await this.AfterSaveAsync(entity, transaction);
                    transaction.Commit();
        ...
        public async Task<IEnumerable<T>> GetAllAsync<T>(Type type, PagingRequest request)
        {
            var response = new PagingResponse();
            var commandText = BaseEntityQuery.BaseBL_GetPaging;
            GetPagingCommandText(ref commandText);
        ...
        public async Task<List<T>> GetAllObjectAsync<T>(Type type, PagingRequest request)
        {
            var res = await GetAllAsync<T>(type, request);
            return res.ToList();
        }
        ...
        public virtual async Task<PagingResponse> GetPagingAsync(Type type, PagingRequest pagingRequest)
        {
            ...
            _logger.LogInfo(JsonConvert.SerializeObject(result));
            return response;
        }

        public async Task<int> GetTotalCount(Type type)
        {
            var sql = Utils.GetStringQuery("BaseBL_GetTotalCount");
            var instance = (BaseEntity)Activator.CreateInstance(type);
            var modelName = instance.GetTableConfig()?.TableName ?? instance.GetType().Name;
            sql = sql.Replace("{modelName}", modelName);
            var total = await DL.ExecuteScalarAsyncUsingCommandText<int>(sql);
            return total;
        }
