...
namespace BaseCoreService.DL
{
    public partial class BaseDL : IBaseDL
    {
        ...
        public async Task<T> ExecuteScalarAsyncUsingCommandText<T>(string commandText, IDictionary<string, object>? parameters = null, IDbConnection? connection = null, IDbTransaction? transaction = null)
        {
            T result = default(T);
            var cd = new CommandDefinition();
            ...
            {
                var con = transaction != null ? transaction.Connection : connection;
                con ??= GetDbConnection(string.Empty);
                if (con != null)
                {
